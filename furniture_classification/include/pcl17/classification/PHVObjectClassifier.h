...
public:
	...
	PHVObjectClassifier() :
			subsampling_resolution_(0.02f), mls_polynomial_fit_(false), mls_polynomial_order_(
					2), mls_search_radius_(0.05f), min_points_in_segment_(100), rg_residual_threshold_(
					0.05f), rg_smoothness_threshold_(40 * M_PI / 180), fe_k_neighbours_(
					10), num_clusters_(40), num_neighbours_(1), cell_size_(
					0.01), local_maxima_threshold_(0.5f), window_size_(0.3), ransac_distance_threshold_(
					0.01f), ransac_vis_score_weight_(5), ransac_num_iter_(200), icp_treshold_(
					0.03), num_angles_(36), icp_max_iterations_(20), icp_max_correspondence_distance_(
					0.01), debug_(false), debug_folder_(""), mls_(
					new MovingLeastSquares<PointT, PointNormalT>) {
	...
	virtual void addObjectPartialView(PointCloudConstPtr view,
			const std::string & class_name) {
		class_name_to_partial_views_map_[class_name].push_back(
				estimateNormalsAndSubsample(view));

	}
	virtual void addObjectFullModel(PointCloudConstPtr model,
			const std::string & class_name) {
		class_name_to_full_models_map_[class_name].push_back(
				estimateNormalsAndSubsample(model));
	}
	...
	virtual void setScene(PointCloudConstPtr model, float cut_off_distance =
			2.5f) {
		std::vector<int> idx;
		...
		pcl17::getMinMax3D<PointNormalT>(*scene_, min_scene_bound_,
				max_scene_bound_);
	}
	...
	void eval_clustering(const std::string & classname,
			const float search_radius, double &tp, double &fn, double &fp);

	template<class PT, class PNT, class FT>
	friend YAML::Emitter& operator <<(YAML::Emitter& out,
			const PHVObjectClassifier<PT, PNT, FT> & h);

	void eval_clustering_external(const std::string & classname,
			const float search_radius, double &tp, double &fn, double &fp,
...
protected:
	...
	void appendFeaturesFromCloud(PointNormalCloudPtr & cloud,
			const string & class_name, const int i);
	void normalizeFeatures(std::vector<FeatureT> & features);
	void normalizeFeaturesWithCurrentMinMax(std::vector<FeatureT> & features);
	void clusterFeatures(vector<FeatureT> & cluster_centers,
			vector<int> & cluster_labels);
	void vote();
	Eigen::MatrixXf projectVotesToGrid(
			const pcl17::PointCloud<pcl17::PointXYZI> & model_centers,
			int & grid_center_x, int & grid_center_y);
	typename pcl17::PointCloud<PointT>::Ptr findLocalMaximaInGrid(
			Eigen::MatrixXf grid, float window_size);
	vector<boost::shared_ptr<std::vector<int> > >
	findVotedSegments(typename pcl17::PointCloud<PointT>::Ptr local_maxima_,
			const string & class_name, float window_size);
	void fitModelsWithRansac(
	...
	void generateVisibilityScore(vector<PointNormalCloudPtr> & result_,
			vector<float> & scores_);
	bool intersectXY(const pcl17::PointCloud<PointNormalT> & cloud1,
...
YAML::Emitter& operator <<(YAML::Emitter& out,
		const pcl17::PHVObjectClassifier<PT, PNT, FT> & h) {
	...
	out << YAML::Key << "icp_max_iterations";
	out << YAML::Value << h.icp_max_iterations_;

	out << YAML::Key << "icp_max_correspondence_distance";
	out << YAML::Value << h.icp_max_correspondence_distance_;
	...
	BOOST_FOREACH(M v, h.class_name_to_full_models_map_) {
		...
		for (size_t i = 0; i < v.second.size(); i++) {
			...
			pcl17::io::savePCDFileASCII(h.database_dir_ + ss.str(),
					*v.second[i]);
			out << ss.str();
...
YAML::Emitter& operator <<(YAML::Emitter& out,
		const pcl17::PointCloud<pcl17::ESFSignature640> & cloud) {
	out << YAML::Flow << YAML::BeginSeq;
...
void operator >>(const YAML::Node& node,
		pcl17::PointCloud<pcl17::ESFSignature640> & cloud) {
	cloud.clear();
...
YAML::Emitter& operator <<(YAML::Emitter& out,
		const pcl17::PointCloud<pcl17::VFHSignature308> & cloud) {
	out << YAML::BeginSeq;
	for (size_t i = 0; i < cloud.points.size(); i++) {
		out << cloud.points[i];
	}
...
void operator >>(const YAML::Node& node,
		pcl17::PointCloud<pcl17::VFHSignature308> & cloud) {
	cloud.clear();
...
YAML::Emitter& operator <<(YAML::Emitter& out,
		const pcl17::PointCloud<PointT> & cloud) {
	out << YAML::BeginSeq;
	for (size_t i = 0; i < cloud.points.size(); i++) {
...
YAML::Emitter& operator <<(YAML::Emitter& out,
	...
	for (typename map<FeatureT, map<string, pcl17::PointCloud<PointT> > >::const_iterator it =
		...
		for (typename map<string, pcl17::PointCloud<PointT> >::const_iterator it2 =
				class_centroid_map.begin(); it2 != class_centroid_map.end();
				it2++) {
...
template<class PT, class PNT, class FT>
void operator >>(const YAML::Node& node,
		pcl17::PHVObjectClassifier<PT, PNT, FT> & h) {
	...
	node["icp_max_iterations"] >> h.icp_max_iterations_;
	node["icp_max_correspondence_distance"] >> h.icp_max_correspondence_distance_;
...
template<int N>
void operator >>(const YAML::Node& node,
		pcl17::PointCloud<pcl17::Histogram<N> > & feature_cloud) {
